...
<%
    ...
    System.out.println("G-PIN authResponse in");
    if (request.getRemoteAddr().equals(session.getAttribute("gpinUserIP"))) {
%>
<table>
    <tr>
        <td>중복확인코드(dupInfo)</td>
        <td><%= getSession(session, "dupInfo") %>
        </td>
    </tr>
    <tr>
        <td>개인식별번호(virtualNo)</td>
        <td><%= getSession(session, "virtualNo") %>
        </td>
    </tr>
    <tr>
        <td>이름(realName)</td>
        <td><%= getSession(session, "realName") %>
        </td>
    </tr>
    <tr>
        <td>성별(sex)</td>
        <td><%= getSession(session, "sex") %>
        </td>
    </tr>
    <tr>
        <td>나이(age)</td>
        <td><%= getSession(session, "age") %>
        </td>
    </tr>
    <tr>
        <td>생년월일(birthDate)</td>
        <td><%= getSession(session, "birthDate") %>
        </td>
    </tr>
    <tr>
        <td>국적(nationalInfo)</td>
        <td><%= getSession(session, "nationalInfo") %>
        </td>
    </tr>
    <tr>
        <td>본인인증방법(authInfo)</td>
        <td><%= getSession(session, "authInfo") %>
        </td>
    </tr>
</table>
<%
