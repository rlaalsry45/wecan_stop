...
%>
<%if(request.getParameter("deleteok")!=null){
	if(request.getParameter("deleteok").equals("true")){%>
...
<%if(request.getParameter("modifyok")!=null){
	if(request.getParameter("modifyok").equals("true")){%>
	<script type="text/javaScript" language="javascript">
		window.opener.location.reload();
		alert("수정되었습니다.");
		window.close();
...
function fn_egov_modify_DeptSchdulManage(){
	var vFrom = document.DeptSchdulManageForm;
	vFrom.cmd.v = '';
	vFrom.action = "<c:url value='/admsys/module/schdule/EgovDeptSchdulManageModify.do' />";;
	vFrom.submit();
...
function fn_egov_delete_DeptSchdulManage(){
	...
	if(confirm("삭제 하시겠습니까?")){
		vFrom.cmd.v = 'del';
		vFrom.action = "<c:url value='/admsys/module/schdule/EgovDeptSchdulManageDetail.do' />";
		vFrom.submit();
	}else{
		vFrom.cmd.v = '';
	}
...
<!-- 상단타이틀 -->
<form name="DeptSchdulManageForm" id="DeptSchdulManageForm" action="<c:url v=''/>" method="post">
<!-- ----------------- 상단 타이틀  영역 -->
<table width="100%" cellpadding="8" class="table-search" border="0">
 <tr>
  <td width="100%"class="title_left">
   <img src="<c:url value='/com/art/egovframework/com/cmm/icon/tit_icon.gif'/>" width="16" height="16" hspace="3" align="middle" alt="제목아이콘이미지">&nbsp;센터일정관리 상세보기</td>
 </tr>
</table>
<!-- 줄간격조정  -->
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" class="table-register">
  <tr>
    <th width="20%" height="23" class="required_text" nowrap >일정구분<img src="<c:url value='/com/art/egovframework/com/cmm/icon/required.gif'/>" width="15" height="15" alt="필수입력표시"></th>
    <td width="80%" >
    <c:forEach items="${schdulSe}" var="schdulSeInfo" varStatus="loop">
  ...
  <%-- <tr>
    <th width="20%" height="23" class="required_text" nowrap >중요도<img src="<c:url value='/com/art/egovframework/com/cmm/icon/required.gif'/>" width="15" height="15" alt="필수입력표시"></th>
    <td width="80%" >
  ...
  <tr>
    <th width="20%" height="23" class="required_text" nowrap >센터<img src="<c:url value='/com/art/egovframework/com/cmm/icon/required.gif'/>" width="15" height="15" alt="필수입력표시"></th>
	<td width="80%" >
  ...
  <tr>
    ...
    <td width="80%">
    <c:forEach items="${reptitSeCode}" var="schdulSeInfo" varStatus="loop">
  ...
  <%-- <tr>
    <th width="20%" height="23" class="required_text" nowrap >담당자<img src="<c:url value='/com/art/egovframework/com/cmm/icon/required.gif'/>" width="15" height="15" alt="필수입력표시"></th>
    <td width="80%" >
...
<!-- 첨부파일 테이블 레이아웃 설정 Start..-->
  <tr>
	<th height="23" class="required_text" nowrap>파일첨부<img src="<c:url value='/com/art/egovframework/com/cmm/icon/no_required.gif'/>" width="15" height="15" alt="선택입력표시"></th>
	<td>
...
<!-- 첨부파일 테이블 레이아웃 End.-->
  <tr>
    <th width="20%" height="23" class="required_text" nowrap >노출지역<img src="<c:url value='/com/art/egovframework/com/cmm/icon/required.gif'/>" width="15" height="15" alt="필수입력표시"></th>
	<td width="80%" >
		<c:if test='${resultList[0].exposuretype eq "P"}'><c:out value="${fn:replace(resultList[0].schdulDeptName , crlf , '<br/>')}" escapeXml="false" />(중앙센터승인대기)</c:if>
		<c:if test='${resultList[0].exposuretype eq "C"}'><c:out value="${fn:replace(resultList[0].schdulDeptName , crlf , '<br/>')}" escapeXml="false" />,중앙센터</c:if>
		<c:if test='${resultList[0].exposuretype eq "1"}'><c:out value="${fn:replace(resultList[0].schdulDeptName , crlf , '<br/>')}" escapeXml="false" /></c:if>
		<c:if test='${resultList[0].exposuretype eq "G"}'><c:out value="전국센터 노출" escapeXml="false" /></c:if>
...
<!--
  ...
  <td>&nbsp;</td>
  <td><img src="<c:url value='/com/art/egovframework/com/cmm/button/bu2_left.gif'/>" width="8" height="20" alt="버튼이미지"></td>
  <td background="<c:url value='/com/art/egovframework/com/cmm/button/bu2_bg.gif'/>" class="text_left" nowrap><a href="JavaScript:fn_egov_delete_DeptSchdulManage();"><spring:message code="button.delete" /></a>
  </td>
  <td><img src="<c:url value='/com/art/egovframework/com/cmm/button/bu2_right.gif'/>" width="8" height="20" alt="버튼이미지"></td>
  <td>&nbsp;</td>
  <td><img src="<c:url value='/com/art/egovframework/com/cmm/button/bu2_left.gif'/>" width="8" height="20" alt="버튼이미지"></td>
  <td background="<c:url value='/com/art/egovframework/com/cmm/button/bu2_bg.gif'/>" class="text_left" nowrap><a href="JavaScript:fn_egov_diary_DeptSchdulManage();">일지관리</a>
  </td>
  <td><img src="<c:url value='/com/art/egovframework/com/cmm/button/bu2_right.gif'/>" width="8" height="20" alt="버튼이미지"></td>
  <td>&nbsp;</td>
  <td><img src="<c:url value='/com/art/egovframework/com/cmm/button/bu2_left.gif'/>" width="8" height="20" alt="버튼이미지"></td>
  <td background="<c:url value='/com/art/egovframework/com/cmm/button/bu2_bg.gif'/>" class="text_left" nowrap><a href="JavaScript:fn_egov_modify_DeptSchdulManage();"><spring:message code="button.update" /></a>
  </td>
  <td><img src="<c:url value='/com/art/egovframework/com/cmm/button/bu2_right.gif'/>" width="8" height="20" alt="버튼이미지"></td>
-->
  ...
  <td>&nbsp;</td>
  <td><span class="button"><input type="submit" value="<spring:message code="button.close" />" title="<spring:message code="button.close" />" onclick="window.close();return false;"></span></td>
...
<input name="cmd" type="hidden" value="<c:out v=''/>"/>
</form>
</DIV>
