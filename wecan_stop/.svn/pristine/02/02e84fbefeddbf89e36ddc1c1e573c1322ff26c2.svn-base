...
<title>::지번검색::</title>
<link type="text/css" href="/usr/skin/newpost/css/post.css" rel="stylesheet" />
<script src="/usr/skin/newpost/js/post.js"></script>
 ...
 */
   ...
   function changeAreaList(idx, obj){	
		...
		var url = "/skin/newpost/AjaxRequestXML.jsp?getUrl=http://www.juso.go.kr/getAreaCode.do?" + escape(createParameter());
	...
	function normalSearch(currentPage){
		...
		var keyword = "";
		//var cityText='',countyText='',townText='',riText='',orgCode='232',orgNm='어린이급식관리지원센터';
		var cityText='',countyText='',townText='',riText='',orgCode='${orgCode}',orgNm='${orgNm}';
		...
		url = "/skin/newpost/AjaxRequestXML.jsp?getUrl="+escape("http://www.juso.go.kr/link/search.do?extend=false&mode=jibun_search&searchType=location_jibun&topTab=1&engineCtpNm="+encodeURI(cityText)+"&engineSigNm="+encodeURI(countyText)+"&engineEmdNm="+encodeURI(townText)+"&engineLiNm="+encodeURI(riText)+"&engineBdMaSn="+encodeURI(form.bun1.value)+"&engineBdSbSn="+encodeURI(form.bun2.value)+"&engineMtYn="+encodeURI(form.san1.value)+"&currentPage="+currentPage+"&orgCode="+orgCode+"&orgNm="+encodeURI(orgNm));
	...
	function normalSearch2(currentPage){
		...
		var form = document.check;
		form.currentPage.value =currentPage;
		//var orgCode='232',orgNm='어린이급식관리지원센터';
		var orgCode='${orgCode}',orgNm='${ogrNm}';

		currentPage=parseInt((currentPage/10)+1);
		
		url = "/skin/newpost/AjaxRequestXML.jsp?getUrl="+escape("http://www.juso.go.kr/link/search.do?extend=false&mode=jibun_search&searchType=location_jibun&topTab=1&engineCtpNm="+encodeURI(form.engineCtpNm.value)+"&engineSigNm="+encodeURI(form.engineSigNm.value)+"&engineEmdNm="+encodeURI(form.engineEmdNm.value)+"&engineLiNm="+encodeURI(form.engineLiNm.value)+"&engineBdMaSn="+form.engineBdMaSn.value+"&engineBdSbSn="+form.engineBdSbSn.value+"&engineMtYn="+form.engineMtYn.value+"&currentPage="+currentPage+"&orgCode="+orgCode+"&orgNm="+encodeURI(orgNm));
	...
	*/
	 ...
	 function setOpenerValue(newaddr){
		 var result = newaddr.split(","); 
		 window.opener.document.getElementById("useraddr").value = result[0];
		 window.opener.document.getElementById("useraddr2").value = document.getElementById("detail").value+" "+result[1];

		 var result2 = result[2].split("-");
		 window.opener.document.getElementById("useraddrno1").value = result2[0];
		 window.opener.document.getElementById("useraddrno2").value = result2[1];
		 window.opener.document.getElementById("useraddr2").focus();
		 window.close();
	 }
	 ...
	 function selectSearch(mode) {
		 if (mode == 'road'){
				document.location.href = "/skin/newpost/road.html";
			}else if(mode =='jibun'){
				document.location.href = "/skin/newpost/jibun.html";
			}else if(mode=='sangho'){
				document.location.href = "/skin/newpost/sangho.html";
			}		
	}
