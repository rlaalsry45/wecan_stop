...
function crtfc(){
	if (checkForm()){
		$.ajax({
             type: "POST"
            ,url: "/frontsys/login/login2ndResult.html"
            ,data: $("#frm").serialize()
            ,dataType: "json"
            ,success: function(data){
            	if (data.resultCode == "success"){
            		location.href = "/?menuno="+sessionStorage.getItem("wecan_np");
		       	}else{
...
<div class="content">
    <div class="cont_wrap">
        <div class="cont">
            <p class="p_t t_center">한국여성인권진흥원 조직문화진단 2차 로그인<br>(구글 OTP 인증하기)</p>
