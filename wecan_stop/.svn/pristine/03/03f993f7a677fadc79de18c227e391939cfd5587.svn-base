...
public class CommonFileController {
    ...
    public void download(HttpServletRequest request, HttpServletResponse response) {
        try {
        	...
        	if ( no > 0 ) {
        		name = commonFileService.retrieveFileNameByNO(no);
        		path = Validator.path(EgovProperties.getPathProperty("Globals.menu.file.location") + System.getProperty("file.separator") + no);
        		Browser.download(request, response, path, name);
        	} else {
        		if ( no == -10 ) {
        			name = request.getParameter("excelname");
        			path = Validator.path(EgovProperties.getPathProperty("Globals.excel.file.location") + System.getProperty("file.separator") + name);
        			Browser.download(request, response, path, name);
        		}
        	}
        } catch (Exception e) {
