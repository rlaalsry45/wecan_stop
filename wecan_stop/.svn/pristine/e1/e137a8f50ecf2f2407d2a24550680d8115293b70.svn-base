$(function(){
    ...
    var acacnt = $("tbody[name=academicbody]").length
    
    for(var j=1;j<=acacnt;j++){
	    var str    = "";
        for(var i = 0 ; i < 80 ; i++ ) {
        	if((year - i)==$("#graduationyear"+j).val()){
        		str += "<option value='"+(year - i)+"'selected >"+(year - i)+"년"+"</option>";
        	}else{
                str += "<option value='"+(year - i)+"'>"+(year - i)+"년"+"</option>";
        	}
	    };
	    $("#graduation"+j).append(str);
    }
    ...
    for(var j=1;j<=careercnt;j++){
	    ...
	    var emonth	 = "";
        for(var i = 0 ; i < 80 ; i++ ) {
        	if((year - i)==$("#syeardata"+j).val()){
        		syear += "<option value='"+(year - i)+"'selected >"+(year - i)+"</option>";
        	}else{
        		syear += "<option value='"+(year - i)+"'>"+(year - i)+"</option>";
        	}
        	if((year - i)==$("#eyeardata"+j).val()){
        		eyear += "<option value='"+(year - i)+"'selected >"+(year - i)+"</option>";
        	}else{
        		eyear += "<option value='"+(year - i)+"'>"+(year - i)+"</option>";
        	}
	    }
        for(var m = 1 ; m <= 12 ; m++ ) {
        	if(m==$("#smonthdata"+j).val()){
        		smonth += "<option value='"+m+"'selected >"+m+"</option>";
        	}else{
        		smonth += "<option value='"+m+"' >"+m+"</option>";
        	}
        	if(m==$("#emonthdata"+j).val()){
        		emonth += "<option value='"+m+"'selected >"+m+"</option>";
        	}else{
   ...
   /* document.getElementById('photofile').onchange=function(){
	...
	frm.submit(function(){
		...
		if($("#work_grade").val() != "7"){ //단체회원이 아닐때만 체크
		...
		frm.attr("action","/skin/member/kpa/do_modify.html");
		return true;
	});
