<%@ page contentType="text/html;charset=utf-8"%>
<%@ include file="/WEB-INF/jsp/zcms/include.jsp" %>
<!--여기서부터 사용하세요-->
<h3 class="ctit">Membership Registration</h3>
<ul class="member-kind en">
	<li class="first kind"><span>Select<br />Membership<br />Type</span></li>
	<li class="name"><span>Real Name<br />Verification</span></li>
	<li class="term"><span>Legal<br />Agreements</span></li>
	<li class="info"><span>Enter Member<br />Information</span></li>
	<li class="email"><span>E-mail<br />Verification</span></li>
	<li class="finish on"><span>Registration<br />Complete</span></li>
</ul>
<h4 class="sctit">Registration Complete</h4>
<p class="text">E-mail verification is required for the protection of your personal information.</p>
<div class="finish">
	Your registration is complete.<br />Please log in with your registered e-mail address to use the website.
</div>
<div class="btn-c">
	<a href="/"><img src="/usr/image/common/btn/btn_main.gif" alt="To the Main Page" /></a>
</div>
<!--/여기까지 사용하세요-->