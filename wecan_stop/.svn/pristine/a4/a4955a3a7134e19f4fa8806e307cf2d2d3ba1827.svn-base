...
<div class="webtong_gallery03 webzine">
		...
		<div class="card_wrap">
			<c:if test="${not empty data.filelist}">
				<c:forEach items="${data.filelist}" var="datafile" varStatus="status">
					<c:set var="file_path" value="${datafile.fsave }"/>
				</c:forEach>
		    </c:if>
			<a class="thumbnail" href="${file_path }" target="_blank">
			...
			<a class="title center" href="${file_path }" target="_blank">${data.bbstitle}</a>
			<c:if test="${sessionScope.frontAuthPassport.role_w ne '0'}">
			<a href="#none" onclick="return submitForm(this,'view',${data.bbsno});" title="${data.bbstitle}" style="color:red;float: right">[수정]</a>
			</c:if>
		</div>
	</c:forEach>
