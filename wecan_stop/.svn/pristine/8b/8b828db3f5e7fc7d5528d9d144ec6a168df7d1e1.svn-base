...
<script type="text/javascript">
	function select_user(userno, userid, username,dept_nm,userphone,useremail,position){
		//console.log(userno,userid,username);
		window.opener.select_user(userno, userid, username,dept_nm,userphone,useremail,position);
...
<c:if test="${noauth ne 'true' }">
	...
	<div id="pop-wrap">
							...
							<tbody>
								<c:forEach items="${list}" var="each" varStatus="loop">
									<tr>
										...
										<td><c:out value='${each.usercompanyname}' />/<c:out value='${each.dept_full_nm}' /></td>
										...
										<td>
											...
											<a href="javascript:select_user('${each.userno}','${each.userid}','${each.username}','${each.usercompanyname}/${each.dept_full_nm}','${each.usermobile}','${each.useremail}','${each.usercompanystatus}')">유저선택</a>
										</td>
									</tr>
								</c:forEach>
		...
		<!--<a href="javascript:void(0)" onclick="window.opener.closePopup(); window.close();" id="close2" class="btn-close"><img src="/usr/image/popup/btn_close02.gif" alt="닫기" /></a>-->
