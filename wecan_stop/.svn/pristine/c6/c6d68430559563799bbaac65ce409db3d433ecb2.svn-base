...
<body>
	<div id="pop-wrap">
		...
		<c:if test="${lang==0}">
			<a href="javascript:void(0)" class="move"><img onclick ="goArchv('http://kf.or.kr/?menuno=476&type=view&archv_no=${archv.archv_no}&path=${archv.path}&tab=1&lang=0')" src="/usr/image/common/btn/btn_move02.gif" alt="아카이브로 이동" /></a>
		</c:if>
		<c:if test="${lang==1}">
			<a href="javascript:void(0)" class="move"><img onclick ="goArchv('http://en.kf.or.kr/?menuno=636&type=view&archv_no=${archv.archv_no}&path=${archv.path}&tab=1&lang=1')" src="/usr/image/common/btn/btn_move02.gif" alt="아카이브로 이동" /></a>
		</c:if>
