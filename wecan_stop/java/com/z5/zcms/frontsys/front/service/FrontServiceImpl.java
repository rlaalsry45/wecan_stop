        ...
        try {
           ...
           print("host = " + host);
            ...
            if (menuNo < 1 && StringUtils.equalsIgnoreCase(site.getRedirectuse(), "y")) {
                String redirect = "redirect:" + request.getScheme() + "://" + Validator.path(host + "/?menuno=" + site.getRedirectmenuno());
                print(redirect);
                return redirect;
            ...
            /*if(scs.getLocation_use_yn() == 1){
                ...
                if (ztags.size() > 0) {
                    ...
                    if(menuNo == 229) {
                    	...
                    	String menutop = this.getHttpURLConnectionContents(menutopJspFilePath, session, request);
                    	buf.append(menutop);
                    	frontMenuService.getMenuno246(request, response, session);
                    }
                    /* --여성폭력 사이버 상담 메뉴 e-- */
