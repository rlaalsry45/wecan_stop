...
import com.z5.zcms.admsys.cyberCounsel.web.DownloadView;
...
public class ArchvRegController {
    ...
    @RequestMapping(value={"/archv/data/list.html","/archv/data/noticelist.html"
    						,"/archv/data/systemInquirieslist.html"})
    public String list(@ModelAttribute ArchvVO archvVO, HttpServletRequest request, Model model) {
        ...
        } else if ( request.getServletPath().equals("/admsys/archv/data/systemInquirieslist.html") ) {
        	catgry_cd = "401";
        	String dashNo = request.getParameter("dashNO");
        	model.addAttribute("dashNO", dashNo);
        }
    ...
    @RequestMapping(value = {"/archv/data/insert.html","/archv/data/noticeInsert.html"
    						,"/archv/data/systemInquiriesInsert.html"})
    public String insert(HttpServletRequest request, Model model) {
        ...
        model.addAttribute("reg_nm", zUserVo.getChargername());
        ...
        } else if ( request.getServletPath().equals("/admsys/archv/data/systemInquiriesInsert.html") ) {
    ...
    @RequestMapping(value = { "/archv/data/update.html"
			...
			, "/archv/data/systemInquiriesUpdate.html", "/archv/data/systemInquiriesView.html"})
    public String update(@ModelAttribute ArchvVO archvVO, HttpServletRequest request, Model model) {
		...
		if ( request.getServletPath().equals("/admsys/archv/data/noticeView.html") 
			|| request.getServletPath().equals("/admsys/archv/data/systemInquiriesView.html") ) {
			try {
				this.archvRegService.updateHits(Integer.valueOf(archvVO.getArchv_no()));
			} catch (Exception e1) {
				e1.printStackTrace();
			}
		}
		...
		if (data != null) {
			...
			} else if ( request.getServletPath().equals("/admsys/archv/data/systemInquiriesUpdate.html") ) {
				model.addAttribute("catgry_cd", "401");
				retURL = "/zcms/admsys/archv/data/update";
			} else if ( request.getServletPath().equals("/admsys/archv/data/systemInquiriesView.html") ) {
				List<ArchvVO> commentList = null;
				try {
					commentList = this.archvRegService.getArchvComment(Integer.valueOf(archvVO.getArchv_no()));
				} catch (Exception e1) {
    ...
    @RequestMapping(value = {"/archv/data/insert.html", "/archv/data/update.html"
    						...
    						, "/archv/data/systemInquiriesUpdate.html", "/archv/data/systemInquiriesInsert.html"}, method = RequestMethod.POST)
...
//
            ...
            if (opt_no != 7) { // 첨부파일이 있을 경우(동영상 / 기타 제외)
            	...
            	if ("update".equals(request.getParameter("cmd_type")) ) {
            		this.archvRegService.delArchvFiles(Integer.valueOf(archvVO.getArchv_no()));
    			}
...
//        }
        ...
        if ( request.getServletPath().equals("/admsys/archv/data/noticeUpdate.html") 
        	...
        	returnURL =  "redirect:/admsys/archv/data/noticelist.html";
        		...
        		|| request.getServletPath().equals("/admsys/archv/data/systemInquiriesInsert.html")) {
        	returnURL =  "redirect:/admsys/archv/data/systemInquirieslist.html";
        }else {
    ...
    // 파일 다운로드
 	@RequestMapping(value = "/archv/data/fileDownload.html")
 	...
 	{
 		String dir = EgovProperties.getPathProperty("Globals.archive");
 		try {
 			DownloadView fileDown = new DownloadView(); //파일다운로드 객체생성
 			fileDown.filDown(req, res, dir + "/" , file_nm, document_nm); //파일다운로드 
