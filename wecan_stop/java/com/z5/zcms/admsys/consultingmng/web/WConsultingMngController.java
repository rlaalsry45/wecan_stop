...
public class WConsultingMngController {
    ...
    public WConsultingMngVo picdigmng_registration(@ModelAttribute("WConsultingMngVo") WConsultingMngVo reqVo,
    	...
    	try {
			...
			// manager insert
			if (tmL.size() > 0) {
				WOrgCultureDigMngVo tReqVo = new WOrgCultureDigMngVo();
    ...
    public String list(
		...
		accessLogVo.setAction("만족도조사 조회");
    ...
    @RequestMapping(value = "satisfaction/result.html")
    public String result(@ModelAttribute("satisfactionResultVo") SatisfactionResultVo satisfactionResultVo, @ModelAttribute("satisfactionVo") SatisfactionVo satisfactionVo, Model model, HttpServletRequest req) throws Exception {
	
    	String userId = SecuritySessionUtil.getUserid(req);
     	AccessLogVo accessLogVo = new AccessLogVo();
 		accessLogVo.setIp(req.getRemoteAddr());
 		accessLogVo.setAction("만족도조사결과 조회");
 		accessLogVo.setRegId(userId);
 		ftpService.registration_accessLog(accessLogVo);
 		
    	if(!StringUtils.isEmpty(req.getParameter("pageIndex"))) {
    		satisfactionVo.setSatisfactionno(satisfactionResultVo.getSatisfactionno());
    	}
    	...
    	SatisfactionResultVo askno = this.satisfactionService.getAsknoCnt(satisfactionResultVo);
        model.addAttribute("askno", askno);
        
        List<SatisfactionResultVo> answerList = this.satisfactionService.getAnswerList(satisfactionResultVo);
        model.addAttribute("answerList", answerList);
        
    	DataTable input = new DataTable(req);
    	satisfactionService.index_satisfaction(satisfactionResultVo, input, model);
     ...
     *
     * @param model
     * @return "/zcms/admsys/consultingmng/familyViolence.html"
     * @throws Exception
     */    
    ...
    public String familyViolence(@ModelAttribute("WCounselReviewVo") WFamilyViolenceVo reqVo, Model model, HttpServletRequest req)
    	...
    	AccessLogVo accessLogVo = new AccessLogVo();
		accessLogVo.setIp(req.getRemoteAddr());
		accessLogVo.setAction("구)가정폭력감수성진단 조회");
		accessLogVo.setRegId(userId);
		ftpService.registration_accessLog(accessLogVo);
		
    	DataTable input = new DataTable(req);
    	wCyberCounselService.index_familyViolence(reqVo, input, model);
        return "/zcms/admsys/consultingmng/familyViolence";
    }    
     ...
     *
     * @param model
     * @return "/zcms/admsys/consultingmng/counselReview.html"
     * @throws Exception
     */
    ...
    public String counselReview(@ModelAttribute("WCounselReviewVo") WCounselReviewVo reqVo, Model model, HttpServletRequest req)
    	...
    	AccessLogVo accessLogVo = new AccessLogVo();
		accessLogVo.setIp(req.getRemoteAddr());
		accessLogVo.setAction("구)만족도조사결과 조회");
		accessLogVo.setRegId(userId);
		ftpService.registration_accessLog(accessLogVo);
		
    	DataTable input = new DataTable(req);
    	wCyberCounselService.index_counselReview(reqVo, input, model);
        return "/zcms/admsys/consultingmng/counselReview";
    }
     ...
     *
     * @param model
     * @return "/zcms/admsys/consultingmng/satisfaction/excel.html"
     * @throws Exception
     */
    @RequestMapping(value = "satisfaction/excel.html", method = {RequestMethod.POST,RequestMethod.GET})
	@ResponseBody
    public Map<String,Object> satisfaction_excel(@ModelAttribute("SatisfactionResultVo") SatisfactionResultVo reqVo, Model model, HttpServletRequest req) throws Exception {
	  ...
	  try{
		  	...
		  	reqVo.setEdate(reqVo.getEdate().replaceAll("-", ""));
	  	  	List<SatisfactionResultVo> list = satisfactionService.satisfaction_excel(reqVo);
