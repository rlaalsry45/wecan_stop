...
public class SatisfactionController {
    ...
    public String list(
        ...
        if (sdate.equals("") && edate.equals("")) {
        	zSatisfactionVo.setCond1("");
        } else {
        	zSatisfactionVo.setCond1(input.get("cond1"));
        }
        if (keyword.equals("")) {
        	zSatisfactionVo.setCond2("");
        } else {
        	zSatisfactionVo.setCond2(input.get("cond2"));
        }

        zSatisfactionVo.setSdate(input.get("sdate").replaceAll("-", ""));
        zSatisfactionVo.setEdate(input.get("edate").replaceAll("-", ""));
        zSatisfactionVo.setKeyword(input.get("keyword"));
        zSatisfactionVo.setM(m);
        zSatisfactionVo.setN(n);

        int total = this.satisfactionService.listCount(zSatisfactionVo);
        input.put("pageSize", pageSize);
        input.put("total", total);
        input.put("pageMax", (int) Math.ceil((double) total / pageSize));

        List<ZSatisfactionVo> list = this.satisfactionService.getSatisfactionList(zSatisfactionVo);
