$(function () {
	...
	$.fn.inputFilter = function(inputFilter) {
		return this.on("input keydown keyup mousedown mouseup select contextmenu drop", function() {
			if (inputFilter(this.value)) {
				...
				//if (this.value.length==$(this).attr("maxLength")) $("input").eq($("input").index(this)+1).focus();
    ...
    $.fn.submitForm = function (f, a, n) {
        ...
        if (a == "write" || a == "edit" || a == "reply") {
			if (""==$.trim($(":input[id='bbstitle']", frm).val())){
        		alert("제목을 입력해 주세요.");
        		$(":input[id='bbstitle']", frm).focus();
        		return false;
			}
			if (""==$.trim($(":input[id='bbsusername']", frm).val())){
        		alert("성명을 입력해 주세요.");
        		$(":input[id='bbsusername']", frm).focus();
        		return false;
			}
			if (""==$.trim($(":input[id='bbsusermobile2']", frm).val())){
        		alert("연락처를 입력해 주세요.");
        		$(":input[id='bbsusermobile2']", frm).focus();
        		return false;
			}
			else{
        		if ($.trim($(":input[id='bbsusermobile2']", frm).val()).length<4){
					alert("4자리 연락처를 입력해 주세요.");
					$(":input[id='bbsusermobile2']", frm).focus();
					return false;
				}
			}
			if (""==$.trim($(":input[id='bbsusermobile3']", frm).val())){
        		alert("연락처를 입력해 주세요.");
        		$(":input[id='bbsusermobile3']", frm).focus();
        		return false;
			}
			else{
        		if ($.trim($(":input[id='bbsusermobile3']", frm).val()).length<4){
					alert("4자리 연락처를 입력해 주세요.");
					$(":input[id='bbsusermobile3']", frm).focus();
					return false;
				}
			}
			if (""==$.trim($(":input[id='etc7']", frm).val())){
        		alert("신고대상(성명)을 입력해 주세요.");
        		$(":input[id='etc7']", frm).focus();
        		return false;
			}
			if (""==$.trim($(":input[id='etc8']", frm).val())){
        		alert("소속부서를 입력해 주세요.");
        		$(":input[id='etc8']", frm).focus();
        		return false;
			}
        	if (!$(":input[id='agree']", frm).is(':checked')){
        		alert("[개인정보 수집 및 이용목적, 보유 및 이용기간에 동의해 주세요.]");
        		$(":input[id='agree']", frm).focus();
        		return false;
        	}
        	else{
	        	frm.setHidden('bbsusermobile', $(":input[name='bbsusermobile1']", frm).val()+"-"+$(":input[name='bbsusermobile2']", frm).val()+"-"+$(":input[name='bbsusermobile3']", frm).val());
	        	frm.setHidden('bbsuseremail', $(":input[name='bbsuseremail1']", frm).val()+"@"+$(":input[name='bbsuseremail2']", frm).val());
    ...
    $(":input[name^=bbsusermobile]").inputFilter(function(value){return /^\d*$/.test(value);});
});
