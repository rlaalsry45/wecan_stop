$(function () {
    ...
    $.fn.setHidden = function (n, v) {
        if ($("input[name='" + n + "']", $(this)).length > 0) {
            $("input[name='" + n + "']", $(this)).val(v);
        } else {
            $("<input>").attr("type", "hidden").attr("name", n).val(v).appendTo($(this));
        }
    };
    ...
    $.fn.submitForm = function (f, a, n) {
        ...
        if (a == "write" || a == "edit" || a == "reply") {
            ...
            if (n.split("^")[0] == '0') {
                ...
                obj.remove();
                $(f).prev().prev().val('No seleted file!');
            }
            else {
            ...
            if ($(f).nextAll("select[name^='cates']").length > 0) {
                ...
                $.each($(f).nextAll("select[name^='cates']"), function (idx, item) {
                    if ($("option:first", $(item)).val() != '') {
                        $("option", $(f).nextAll("select[name^='cates']")).remove();
                    } else {
...
try {
	...
	//썸네일이미지 삭제
	function removeImg1() {
	    $("#thumb_file").val('No selected file!');
