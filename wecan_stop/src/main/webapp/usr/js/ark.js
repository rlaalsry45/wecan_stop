...
var arkPath = "/usr/image/search";					// 자동완성 경로 (이미지)
var transURL = "/ark.html";						// trans 페이지의 URL을 설정한다.
var tempQuery = "";
...
var IE7_TOP_OFFSET = -63 + 100 - 36;				// IE7 일 경우 TOP 옵셋 값 오차 조정
var IE7_LEFT_OFFSET = -18 - 300 + 34;				// IE7 일 경우 LEFT 옵셋 값 오차 조정
...
/** ARK 구성요소의 위치 및 크기를 아래 변수를 통해 조정함. **/
var arkWidth = 37 - 38;							// 자동완성 전체 넓이 값을 설정한다(변동폭).
var arkTop = 29 + 4 - 64;							// 자동완성 상단에서의 위치 값을 설정한다.
var arkLeft = -3 - 140 - 123 + 266;					// 자동완성 왼쪽에서의 위치 값을 설정한다.
var arkImgTop = 26 + 3 + 8;						// 자동완성 화살표 이미지의 상단에서 위치 값을 설정한다.
var arkImgLeft = 26 - 160 - 30;					// 자동완성 화살표 이미지의 왼쪽에서 위치 값을 설정한다.
...
function setArkFooter() {
	...
	if (isArk && $(queryId).val() != "" && (totalFwCount + totalRwCount) > 0 && isListShow) {
		...
		if (g_oConvert == "fw") {
			str += "<label OnClick=\"onConvert('rw');\" style=\"cursor:pointer; color:#888888;\">끝단어 더보기</label>";
		} else if (g_oConvert == "rw") {
			str += "<label OnClick=\"onConvert('fw');\" style=\"cursor:pointer; color:#888888;\">첫단어 더보기</label>";
		}
	...
	if (isArk) {
		...
		if ($(queryId).val() != "" && isListShow) {
			str += "<label id=\"arkOff\" onmouseover=\"previewShow(event, this,'tooltip02');\" OnClick=\"setArkOff();\" style=\"cursor:pointer; color:#888888;\">기능끄기</label>";
			...
			str += "<label OnClick=\"showArk();\" style=\"cursor:pointer; color:#888888;\">닫기</label>";
			isListShow = true;
		}

		str += "</div>";
	} else {
		str += "<div class=\"right\" style=\"padding:8px 0 0 0; color:#888888;\"><label OnClick=\"showArk();\" style=\"cursor:pointer; color:#888888;\">닫기</label></div>";
	}
