...
<table class="write_style_1">
	<caption>쓰기 페이지입니다. 공지여부, 작성자, 제목, 내용, 첨부파일로 구성된 테이블입니다.</caption>
	<colgroup>
		<col width="12%">
		<col width="auto">
	</colgroup>
	<tbody>
		...
		<tr>
			<th scope="row"><label for="bbsusername">작성자</label></th>
			<td><c:if test="${sessionScope.frontAuthPassport.role_m_nm ne '0'}"><input name="bbsusername" title="작성자" class="input_1" id="bbsusername" type="text" value="${detail.bbsusername}"></c:if><c:if test="${sessionScope.frontAuthPassport.role_m_nm eq '0'}">${detail.bbsusername}<input type="hidden" name="bbsusername" value="${detail.bbsusername}"></c:if></td>
		</tr>
		<tr>
			<th scope="row"><label for="bbstitle">제목</label></th>
			<td><input type="text" name="bbstitle" id="bbstitle" title="제목" class="input_1" value='<c:if test="${act=='reply'}">[RE]</c:if>${detail.bbstitle}'></td>
		</tr>
		...
		<tr>
			...
			<td>
                <c:choose>
                    ...
                    <c:otherwise>
						<textarea id="bbsconts" name="bbsconts" title="내용을 입력하세요" rows="10" cols="10" style="width:90%"><c:out value='${detail.bbsconts}' escapeXml="false"/></textarea>
                    </c:otherwise>
                </c:choose>			
			</td>
		</tr>
		<tr>
			<th scope="row"><label for="file2">썸네일</label></th>
			<td>
				<img name="priview1" id="preview1" width="161" height="122" src="/usr/upload/board_thumb/${tblname}/${detail.place}" onError="this.src='/usr/upload/board/noimg.gif'" alt="${data.bbstitle}"/></br>
				<input type='file' name='thumb' id='thumb'/>
				<input type="image" style="border:0px;" src='/usr/skin/board/${skin}/image/btn_delete03.gif' alt='썸네일 삭제' onclick="removeImg1()" />
