...
$(document).ready(function(){
	...
	$('.userAgree_btn').click(function() {
        $( '.userAgree' ).slideToggle( "fast")
    });
...
function checkForm() {
    with (document.frm) {
		...
		if($("input[name='agree_check']:checked").val() == "N") {
			alert("개인정보 수집·이용 내역에 동의하세요.");
	        $("#agree_check1").focus();
	        return false;
		}
	...
	if (checkForm()){
		...
		$.ajax({
            ...
            ,data: "userid="+$("#userid").val()+"&username="+$("#username").val()+"&email="+$("#email").val()
						+"&passwd="+$("#passwd").val()+"&mobile="+$("#mobile").val()+"&userGubun=2&agree_check="+$("input[name='agree_check']:checked").val()
            ,dataType: "json"
...
function emailCrtfc() {
	if (dCheckForm()){
		$.ajax({
            ...
            ,data: "userid="+$("#userid").val()+"&userauth=5"
            ,dataType: "json"
			,async: false
            ,success: function(data){
				if (data.resultCode == "success"){
