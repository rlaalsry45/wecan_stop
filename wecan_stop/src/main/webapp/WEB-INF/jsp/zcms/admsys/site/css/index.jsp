            ...
            <div id="contents">
                <form:form modelAttribute="zcssVo" name="frm" method="post">
                    <div class="contents_top">
						<div class="location">
							<a href="/admsys/dashboard/index.html">HOME</a> <a href="/admsys/site/site/index.html">시스템관리</a> <strong>CSS 관리</strong>
						</div>
						<div class="TopSearch">
							<span>SEARCH AREA</span>
							<select style="height:27px;" name="cond2">
								<option value="csstitle" <c:if test="${input.cond2=='csstitle'}"><c:out value='selected' /></c:if>>CSS명</option>
								<option value="cssfilesave" <c:if test="${input.cond2=='cssfilesave'}"><c:out value='selected' /></c:if>>파일명</option>
							</select>
					...
					<ul class="homepagebbs">
						<li class="bg"><h3 class="sub">CSS 목록</h3><a class="btmore03" href="insert.html">+ CSS 등록</a></li>
						<li>
						<div class="top_bt">
                        ...
                        <div class="main_table">
                            ...
                            <table class="main_table1" border="1" cellspacing="1" cellpadding="2" width="100%" summary="CSS리스트">
                                ...
                                <thead>
                                    <tr>
                                        <th>
                                            <input class="bor_none" id="batch" type="checkbox" onclick="javascript:checkAll('','cssno')" />
                                        </th>
                                        <th>번호</th>
                                        <th>CSS명</th>
                                        <th>파일명</th>
                                ...
                                <tbody>
                                    <c:forEach items="${list}" var="each" varStatus="loop">
                                        <tr>
                                            <td>
                                                <input class="bor_none" name="cssno" value="${each.cssno}" type="checkbox" />
                                            </td>
                                            <td>
                                                <c:out value='${input.total-(input.pageIndex-1)*input.pageSize-loop.index}' />
                                            </td>
                                            <td>
                                                <c:out value='${each.csstitle}' />
                                            </td>
                                            <td>
                                                <c:out value='${each.cssfilesave}' />
                                            </td>
                                            <td>
                                                <c:out value='${fn:replace(each.sitetitle,",","<br>")}' escapeXml="false" />
                                            </td>
                                            <td>
                                                <fmt:parseDate value="${each.cssdatereg}" pattern="yyyyMMddHHmmss" var="isoDate" />
                                                <fmt:formatDate type="both" value="${isoDate}" pattern="yyyy-MM-dd HH:mm:ss" />
                                            </td>
                                            <td>
                                                <fmt:parseDate value="${each.cssdatemod}" pattern="yyyyMMddHHmmss" var="isoDate" />
                                                <fmt:formatDate type="both" value="${isoDate}" pattern="yyyy-MM-dd HH:mm:ss" />
                                            </td>
                                            <td>
							...
							<p class="notification_right01">
								<img alt="!" src="/cms/image/excla.gif"> 1. 상단의 CSS 등록 버튼을 클릭하여 웹페이지 제작에 사용할 css를 등록합니다.
								<br>
								<img alt="!" src="/cms/image/excla.gif"> 2. 미리 작업된 내용을 직접입력으로 적용할 수 있습니다. 파일 등록의 경우 선택된 파일의 내용을 자동으로 불러옵니다.
								<br>
								<img alt="!" src="/cms/image/excla.gif"> 3. "메뉴/콘텐츠" 관리에서 페이지 추가 등록 시 css 관리에 등록한 css가 표기되어 선택 후 사용합니다.
							</p>
                            <!---/게시판 영역------------------------->
                        </div><!--/main_table-->
						</li>
