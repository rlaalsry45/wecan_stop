...
function linkPage(pageNo){
	document.listForm.pageIndex.v = pageNo;
...
function fn_egov_search_DeptSchdulManage(){
	...
	vFrom.action = "<c:url value='/cop/smt/sdm/EgovDeptSchdulManageList.do' />";
	vFrom.submit();
...
function fn_egov_open_Popup(cnt, schdulId){
	...
	opener.document.getElementById("schdulId").v = schdulId;
	opener.document.getElementById("schdulCn").v = document.getElementById("iptText_"+ cnt).v;
...
<form name="listForm" id="listForm" action="<c:url v=''/>" method="post">
<table width="100%" cellpadding="1" class="table-search" border="0">
 <tr>
  <td class="title_left">

   <img src="<c:url value='/com/art/egovframework/com/cmm/icon/tit_icon.gif'/>" width="16" height="16" hspace="3" align="absmiddle" alt="제목아이콘이미지">&nbsp;부서일정관리 목록</td>
  <th>
  </th>
  <td width="110px">
   	<select name="searchCondition" class="select" style="width:100%" title="검색조건선택">
		   <option v=''>--선택하세요--</option>
...
<c:forEach items="${resultList}" var="resultInfo" varStatus="loop">
  <tr>
		<td class="lt_text3">${(searchVO.pageIndex-1) * searchVO.pageSize + loop.count}</td>
		<td class="lt_text3">
    	...
    	<td class="lt_text3L"><a href="#" onClick="JavaScript:fn_egov_open_Popup('${loop.count}', '${resultInfo.schdulId}')">선택</td>
    	<input name="iptText_${loop.count}" id="iptText_${loop.count}" type="hidden" value="${resultInfo.schdulNm}">
  </tr>
</c:forEach>
</tbody>
</table>
