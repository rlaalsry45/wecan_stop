...
<HEAD>
	...
	******************************************************** */
	function fn_egov_regist_DeptSchdulManage(sDate){
		...
		gOpener.action = "<c:url value='/admsys/module/schdule/EgovDeptSchdulManageRegist.do' />";
	...
	******************************************************** */
	function fn_egov_detail_DeptSchdulManage(schdulId){
		...
		gOpener.action ="<c:url value='/admsys/module/schdule/EgovDeptSchdulManageDetail.do' />";
	...
	function fnEgovSchdulSe(setValue) {
		...
		location.href="<%=url%>?year=<%=iNowYear%>&month=<%=iNowMonth%>&day=<%=iNowDay%>&searchCondition=SCHDUL_SE&searchKeyword=" + setValue;
	}
...
<td height="60">
    ...
    <td>
		...
		<td>
			...
			<a href="<%=url%>?year=<%=iNowYear-1%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay%>">
			...
			<td width="80" align="center">
			<%if(iNowMonth > 0 ){ %>
			<a href="<%=url%>?year=<%=iNowYear%>&amp;month=<%=iNowMonth-1%>&amp;day=<%=iNowDay%>">
			<img src="<c:url value='/cms/image/schdule/icon/icoPrev.gif' />" align="middle" style="margin-right:4px;border:0px;" alt="이전으로가기아이콘표시">
			</a>
			<%}%>
			<%=iNowMonth+1%>월
			...
			<td width="80" align="center">
			<%if(iNowDay > 1 ){ %>
			<a href="<%=url%>?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay-1%>">
			<img src="<c:url value='/cms/image/schdule/icon/icoPrev.gif' />" align="middle" style="margin-right:4px;border:0px;" alt="이전으로가기아이콘표시">
			</a>
			<%}%>
			<%=iNowDay%>일
