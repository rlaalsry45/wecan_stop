...
<div class="contents">
	...
	<h3 class="ctit">아카이브</h3>
		...
		<div class="board-view">
			<h4 class="btit">
				<%-- <subs:substringOut str='${archv.title}' length='80' /> --%>
				${archv.title}
			</h4>
			<table class="board-view" summary="작성일, 행사기간, 작성자">
				...
				<tbody>
					...
					<tr>
						<th scope="row">
							<span>
								<c:if test="${lang==0}">문서</c:if>
								<c:if test="${lang==1}">Attach</c:if>
							</span>
						</th>
						<td rowspan="3" colspan="6">

							<c:forEach items="${filelist}" var="filename" varStatus="loop">
								<a href="/front/archv/downloadFile.html?filetype=D&filename=${filename.realname }">${filename.name }
									<c:set var="ext" value="${fn:split(filename.name,'.')}" />
			...
			<ul class="prev-next prev-next2">
				<li class="first">
					...
					<c:if test="${archv.pre_archv_no !=null}">
						<a href="/?menuno=${menuno}&type=view&archv_no=${archv.pre_archv_no}&pageIndex=${input.pageIndex}&path=${filename}&tab=${tab}&eqindex=${eqindex}&lang=${lang}">
				...
				<li>
					...
					<c:if test="${archv.next_archv_no !=null}">
						<a href="/?menuno=${menuno}&type=view&archv_no=${archv.next_archv_no}&pageIndex=${input.pageIndex}&path=${filename}&tab=${tab}&eqindex=${eqindex}&lang=${lang}">
			...
			<div class="btn-c">
				<a href="/?menuno=${menuno}&pageIndex=${input.pageIndex}&path=${filename}&tab=${tab}&eqindex=${eqindex}&lang=${lang}">
