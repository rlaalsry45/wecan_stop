	...
	<div id="pop-wrap">
		...
		<div id="p-container2">
			<div class="board-view popup">
				<div class="vod">
					<iframe  width="640" height="480" src="http://www.youtube.com/embed/${archv.vdo_url}?rel=0&autoplay=1" frameborder="0" allowfullscreen></iframe>
					<!-- <iframe width="420" height="315" src="//www.youtube.com/embed/DbnQI4eW7g8" frameborder="0" allowfullscreen></iframe> -->
				</div>
		...
		<c:if test="${lang==0}">
			<a href="javascript:void(0)" class="move"><img onclick ="goArchv('http://kf.or.kr/?menuno=476&type=view&archv_no=${archv.archv_no}&path=${archv.path}&tab=4&lang=0')" src="/usr/image/common/btn/btn_move02.gif" alt="아카이브로 이동" /></a>
		</c:if>
		<c:if test="${lang==1}">
			<a href="javascript:void(0)" class="move"><img onclick ="goArchv('http://en.kf.or.kr/?menuno=636&type=view&archv_no=${archv.archv_no}&path=${archv.path}&tab=4&lang=1')" src="/usr/image/common/btn/btn_move02.gif" alt="아카이브로 이동" /></a>
		</c:if>
		<%-- 영문아카이브의 경우 운영은 636번, 로컬과 테스트는 641번이므로 테스트 시에는 수정하여 테스트할것 --%>
		<%-- <a href="javascript:void(0)" class="move"><img onclick ="goArchv('http://kf.or.kr/?menuno=${archv_menuno}&type=view&archv_no=${archv.archv_no}&path=${archv.path}&tab=4')" src="/usr/image/common/btn/btn_move02.gif" alt="아카이브로 이동" /></a> --%>
